...
test('honors a supervisor shutdown event with immediate resync and reconnect', async () => { const updates = []; const fetchBundle = jest.fn(async () => ({ bundleVersion: 4 })); const client = createRoutingStream({ endpoint: 'http://vip', fetchBundle, WebSocketImpl: FakeWebSocket, reconnectMs: 1, maxReconnectMs: 1, onUpdate: (event) => updates.push(event) }); await client.connect(); const socket = sockets.at(-1); socket.open(); socket.message({ type: 'routing.shutdown', node: 'writer', reason: 'SIGTERM', reconnectDeadlineMs: 60000, loadBalancerEndpoint: 'http://new-vip' }); await new Promise((resolve) => setImmediate(resolve)); expect(updates).toEqual(expect.arrayContaining([expect.objectContaining({ type: 'routing.shutdown' }), expect.objectContaining({ type: 'routing.resync', bundle: { bundleVersion: 4 } })])); expect(fetchBundle).toHaveBeenCalledWith('http://new-vip'); expect(socket.closeArgs).toEqual([1012, 'supervisor restarting']); expect(client.state().endpoint).toBe('http://new-vip'); client.close(); });
test('does not reconnect after the shutdown deadline expires', async () => { let now = 1000; const client = createRoutingStream({ endpoint: 'http://vip', fetchBundle: async () => ({}), WebSocketImpl: FakeWebSocket, reconnectMs: 1, maxReconnectMs: 1, now: () => now }); await client.connect(); const socket = sockets.at(-1); socket.open(); socket.message({ type: 'routing.shutdown', node: 'writer', reason: 'maintenance', reconnectDeadlineMs: 0, generatedAt: '2099-01-01T00:00:00Z', version: 1 }); now = 1001; await new Promise((resolve) => setImmediate(resolve)); await new Promise((resolve) => setTimeout(resolve, 5)); expect(sockets).toHaveLength(1); client.close(); });
