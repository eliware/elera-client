...
test('rejects unversioned refreshes and ignores older bundles', async () => { const client = await createDb({ primary: profile, bundle, mysqlLib: driver() }); await expect(client.refresh({ ...bundle, bundleVersion: 'v0' })).resolves.toMatchObject({ bundleVersion: 'v1' }); await expect(client.refresh({ ...bundle, bundleVersion: undefined })).rejects.toThrow('bundleVersion'); await client.close(); });
