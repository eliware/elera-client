...
import { createDb, ClusterUnavailableError, ServerUnavailableError } from '../../src/index.mjs';
...
const bundle = {
  ...
  ports: { sql: 3306, http: 8080 }, routes: { primary: [{ host: 'db', port: 3306 }] },
...
test('public entrypoint creates a managed client with only endpoint and token', async () => {
  ...
  expect(ClusterUnavailableError).toBeDefined();
  expect(ServerUnavailableError).toBeDefined();
  await client.close();
});
