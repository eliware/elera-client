...
test('node pool delegates queries, execution, health, connections, and close', async () => { const node = createNodePool({ profile, mysqlLib: driver(), log: { warn: jest.fn() } }); await node.query('SELECT 1'); await node.execute('SELECT 1'); const acquired = await node.getConnection(); acquired.release(); acquired.release(); await expect(node.health()).resolves.toMatchObject({ ok: true, host: 'db' }); await node.close(); });
...
test('passes driver options while omitting acquire timeout and undefined socket paths', async () => { const createPool = jest.fn(() => ({ query: jest.fn(async () => [['ok']]), execute: jest.fn(async () => [['ok']]), getConnection: jest.fn(async () => connection()), end: jest.fn(async () => {}) })); const node = createNodePool({ profile: { ...profile, options: { acquireTimeout: 10, socketPath: undefined, connectionLimit: 2 } }, mysqlLib: { createPool } }); expect(createPool).toHaveBeenCalledWith(expect.objectContaining({ connectionLimit: 2, waitForConnections: true })); expect(createPool.mock.calls[0][0]).not.toHaveProperty('acquireTimeout'); expect(createPool.mock.calls[0][0]).not.toHaveProperty('socketPath'); await node.close(); });
test('preserves an explicitly configured socket path', async () => { const createPool = jest.fn(() => ({ query: jest.fn(async () => [['ok']]), execute: jest.fn(async () => [['ok']]), getConnection: jest.fn(async () => connection()), end: jest.fn(async () => {}) })); const node = createNodePool({ profile: { ...profile, options: { socketPath: '/run/mysqld.sock' } }, mysqlLib: { createPool } }); expect(createPool.mock.calls[0][0].socketPath).toBe('/run/mysqld.sock'); await node.close(); });
...
test('query quarantines retryable failures without requiring a logger', async () => { const bad = Object.assign(new Error('down'), { code: 'ECONNREFUSED' }); const node = createNodePool({ profile, mysqlLib: driver({ query: jest.fn(async () => { throw bad; }) }) }); await expect(node.query('SELECT 1')).rejects.toThrow('down'); expect(node.failures).toBe(1); });

test('close is idempotent and closed nodes cannot recover', async () => {
  const end = jest.fn(async () => {});
  const node = createNodePool({ profile, mysqlLib: { createPool: () => ({ query: jest.fn(async () => [[]]), execute: jest.fn(async () => [[]]), getConnection: jest.fn(async () => ({})), end }) } });
  await node.close();
  await node.close();
  expect(end).toHaveBeenCalledTimes(1);
  expect(node.recover()).toBe('unavailable');
  node.available = true;
  expect(node.available).toBe(false);
});

test('query maps non-retryable failures without quarantining the node', async () => {
  const bad = Object.assign(new Error('invalid'), { code: 'ER_PARSE_ERROR' });
  const node = createNodePool({ profile, mysqlLib: driver({ query: jest.fn(async () => { throw bad; }) }) });
  await expect(node.query('BAD')).rejects.toThrow('invalid');
