...
export function createNodePool({ profile, mysqlLib, log, now = () => Date.now(), quarantineMs = 5000 }) {
  ...
  let lifecycle;
  let closed = false;
  const forceClose = async () => { if (closed) return; closed = true; lifecycle.unavailable = true; await pool.end(); };
  ...
  return { host: profile.host, port: profile.port, weight: Number(profile.weight ?? 100), get available() { return lifecycle.available && now() >= unavailableUntil; }, set available(value) { if (value) { if (!closed) lifecycle.recover(); unavailableUntil = 0; } else lifecycle.drain(); }, get failures() { return failures; }, get state() { return lifecycle.state; }, get active() { return lifecycle.active; }, drain: lifecycle.drain, recover: () => { if (!closed) lifecycle.recover(); return lifecycle.state; }, waitForIdle: lifecycle.waitForIdle, forceClose,
