...
export interface DbClient { query(sql: string, values?: unknown, options?: { route?: 'auto' | 'primary' | 'balanced' }): Promise<unknown>; execute(sql: string, values?: unknown, options?: { route?: 'auto' | 'primary' | 'balanced' }): Promise<unknown>; transaction<T>(callback: (transaction: Pick<DbClient, 'query' | 'execute'>) => Promise<T>): Promise<T>; health(route?: 'primary' | 'balanced'): Promise<unknown>; close(): Promise<void>; bundle(): unknown; availability(): { state: 'available' | 'standalone-unavailable' | 'cluster-unavailable'; routes: { primary: boolean; balanced: boolean } }; drain(host: string, timeoutMs?: number): { wait(): Promise<unknown[]>; forceClose(): Promise<unknown[]> }; nodeStates(): unknown[]; telemetry?: Telemetry; }
