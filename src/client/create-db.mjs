...
import { asSqlError, validateBundle as validateBundleShape, bundleExpired, bundleNeedsRefresh, compareBundleVersions, clientDrainTimeout, createTelemetry } from '@eliware/elera-lib';
...
const completeRoutingUpdate = (active, update, now) => ({
  ...active,
  ...update,
  apiVersion: update.apiVersion ?? active?.apiVersion ?? 'v1',
  application: update.application ?? active?.application,
  database: update.database ?? active?.database,
  identity: update.identity ?? active?.identity,
  credentials: update.credentials ?? active?.credentials,
  writer: update.writer ?? active?.writer,
  readers: update.readers ?? active?.readers ?? [],
  failover: update.failover ?? active?.failover ?? [],
  bundleVersion: update.bundleVersion ?? update.version ?? active?.bundleVersion,
  nodeIdentity: update.nodeIdentity ?? active?.nodeIdentity,
  ports: update.ports ?? active?.ports,
  routes: update.routes ?? active?.routes,
  expiresAt: update.expiresAt ?? active?.expiresAt ?? new Date(now() + 60000).toISOString()
});
...
export async function createDb({ primary, balanced, bundle, credentialProvider, mysqlLib = mysql, log = defaultLog, routing = 'auto', identity, tokenContext, quarantineMs = 5000, drainTimeoutMs = 45000, now = () => Date.now(), telemetry } = {}) {
  ...
  const client = {
    ...
    async refresh(nextBundle) { const candidate = validateBundle(nextBundle); if (bundleExpired(candidate, now())) throw new Error('routing bundle is expired'); if (olderVersion(candidate.bundleVersion, activeBundle?.bundleVersion)) return { bundleVersion: activeBundle?.bundleVersion, refreshRequired: bundleNeedsRefresh(activeBundle, now()) }; const previous = [primaryPool, balancedPool]; const credentials = candidate.credentials; const writer = candidate.writer; const reader = candidate.readers?.[0] ?? candidate.routes.balanced?.[0]; activeBundle = candidate; primaryConfig = validateProfile({ ...primaryConfig, host: writer.host, port: writer.port, user: credentials.username, password: credentials.password, database: candidate.database }, 'primary'); primaryPool = makeRoute('primary', primaryConfig); if (reader) { balancedConfig = validateProfile({ ...primaryConfig, host: reader.host, port: reader.port }, 'balanced'); balancedPool = makeRoute('balanced', balancedConfig); } else if (balancedPool) { for (const node of balancedPool.nodes) node.drain?.(drainTimeoutMs); } await Promise.all(previous.filter(Boolean).filter((pool) => pool !== primaryPool && pool !== balancedPool).map((pool) => pool.close())); return { bundleVersion: activeBundle.bundleVersion, refreshRequired: bundleNeedsRefresh(activeBundle, now()) }; },
     async attachRoutingStream(stream) { if (!stream?.connect) throw new TypeError('routing stream is required'); metrics?.start?.(stream); stream.setTelemetry?.(metrics); stream.setOnUpdate?.(async (event) => { const update = event.type === 'routing.update' ? event : event.type === 'routing.resync' ? event.bundle : undefined; if (update) await client.refresh(completeRoutingUpdate(activeBundle, update, now)); if (event.type === 'routing.drain' || event.type === 'routing.shutdown') for (const pool of [primaryPool, balancedPool].filter(Boolean)) pool.drain(event.node, drainTimeoutMs); if (event.type === 'routing.recovery') for (const pool of [primaryPool, balancedPool].filter(Boolean)) pool.recover(event.node, drainTimeoutMs); }); await stream.connect(); return () => stream.close?.(); },
